...
def prune_leaves(t, vals):
    ...
    if is_leaf(t) and (label(t) in vals):
        ...
        new = [prune_leaves(t, vals) for t in branches(t)]
        return tree(label(t), [t for t in new if t is not None])
...
def totals_tree(m):
    ...
    for b in branches(m):
        if is_side(left):
            return weight(end(left(m)))
        if is_side(right):
            return weight(end(right(m)))
        else:
            return tree(total_weight(m), tree([totals_tree(b) for b in branches(m)]))
# I have no idea why my code is wrong.
