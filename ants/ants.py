...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place is not hive:
            if (current_place.bees and (self.min_range <= steps <= self.max_range)):
    ...
    def action(self, colony):
        ...
        if self.Imposter:
            ...
            while current_place is not None:
                if current_place.ant is not None and current_place.ant not in self.doubled_damage:
                    current_place.ant.damage = 2 * current_place.ant.damage
                    self.doubled_damage += [current_place.ant]
                    if hasattr(current_place.ant, 'ant') and current_place.ant.ant is not None and current_place.ant.ant not in self.doubled_damage:
                        current_place.ant.ant.damage = 2 * current_place.ant.ant.damage
                        self.doubled_damage += [current_place.ant.ant]
